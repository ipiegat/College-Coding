...
int transaction(TPlayer *p1, TPlayer *p2, int amount) {
  // if p1 lands on own property, do nothing
  if (p1->id == p2->id) {
    return 1;
  }
...
int one_round(int m, int n, TPlayer p[], TProperty prop[]) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    // each time player passes go, they get n money
    p[i].balance += (p[i].loc / n) * n;
    p[i].loc %= n;
    // if property empty, player gets it
    if (prop[p[i].loc].owner_id == -1)
      prop[p[i].loc].owner_id = p[i].id;
    // else, they pay up
    else {
      if (!transaction(&p[i], &p[prop[p[i].loc].owner_id],
                       prop[p[i].loc].rent)) {
        return 0;
      }
    }
    // and if they can't, game over

    // If one player cannot pay rent to another player, this function should
    // return 0. The rest of the players will not get chances to play when this
    // happens
  }
  return 1;
}
