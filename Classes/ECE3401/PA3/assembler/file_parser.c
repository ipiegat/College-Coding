				...
				if (data_reached == 0) {
					...
					if (instruction_supported != -1) {
						...
						// I-Type or S-type
							...
							char **reg_store;
							reg_store = malloc(3 * sizeof(char*));
							// reg_store[i] = malloc(8 * sizeof(char));
							if (reg_store == NULL) {
								fprintf(Out, "Out of memory\n");
								exit(1);
							}
						...
						//B-Type	
							...
							if (strcmp(token, "gemm1") == 0)
								fprintf(Out, "00000000000000000000000001111111\n");
							else if (strcmp(token, "gemm2") == 0)
								fprintf(Out, "00000000000000000000000001111110\n");
...
char instruction_type(char *instruction) {
	...
	else if (strcmp(instruction, "gemm1") == 0 ||
         strcmp(instruction, "gemm2") == 0) return 'g';
