...
void *producer(void *t) 
{
    ...
    while(!done)
    {
        ...
        // wait for condition to be set to cond[0] (producer)
        pthread_cond_wait(&pdata->cond[0], &pdata->mutex);

        // lock and save data in struct, new item to be produced
        pthread_mutex_lock(&pdata->mutex);
        pdata->data = v;
        pdata->ready++;

        // signal each consumer that data is ready????
        for (int i = 1; i < NUM_CONSUMERS; i++) {
            pthread_cond_signal(&pdata->cond[i]);
        }
        // unlock for consumers
...
void * consumer(void *t) 
{
    ...
    while (! done) 
	{
        ...
        // wait for cond to be integers 1-11?
        for (int i = 1; i < NUM_CONSUMERS; i++) {
            pthread_cond_wait(&pdata->cond[i], &pdata->mutex);
        }
