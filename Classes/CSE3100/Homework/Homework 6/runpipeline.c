 ...
 *
 * Close file descriptors that are not needed, in both parent and child
 * processes.
 *
...
void start_program(Program *programs, int num_programs, int cur) {
  ...
  // now we know we're the child

  if (pid == 0) {
    // replace stdin with the in fd
    dup2(programs[cur].fd_in, FD_STDIN);

    // replace stdout with out fd
    dup2(programs[cur].fd_out, FD_STDOUT);

    for (int i = 1; i < num_programs; i++) {
      close(programs[i].fd_in);
      close(programs[i - 1].fd_out);
    }
...
void prepare_pipes(Program *programs, int num_programs) {
  ...
  // create pipe filedescriptor
  int fd[2];

  // connect first program to stdin
  ...
  for (int i = 1; i < num_programs; i++) {
    ...
    // connect pipe in to prev program's fd_out
    programs[i - 1].fd_out = fd[PIPEFD_WRITE];
    // connect pipe out to this program's fd_in
    programs[i].fd_in = fd[PIPEFD_READ];
  }

  // connect last program out to stdout
...
int parse_command_line(Program *progs, int max_num_progs, int argc,
                       char **argv) {
  int cur_arg = 1;
  int prog_count = 0;
...
int main(int argc, char **argv) {
  ...
  if (argc <= 1)
    die("Specify at least one program to run. Multiple programs are separated "
        "by --");
