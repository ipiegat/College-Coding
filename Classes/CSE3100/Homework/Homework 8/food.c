...
void add_to_buffer(int item, int col, two_d_buffer *p) {
  ...
  // block while the column is full
...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p) {
  ...
  // block til there is at least one item in each column
  while (p->counts[0] * p->counts[1] * p->counts[2] == 0)
    pthread_cond_wait(&p->produce_cond, &p->mutex);

  // read off the values from the buffer
  *a = p->buf[0][0];
  *b = p->buf[0][1];
  *c = p->buf[0][2];

  // remove the values from the buffer
  for (int col = 0; col < 3; col++) {

    // update the counts
    p->counts[col]--;

    // shift rows down for this column
...
void prepare(int item) { usleep((item + 1) * 100); }
...
void *thread_consume(void *threadarg) {
  ...
  // add n1, n2, and n3 to the linked-list pointed to by p
...
void *thread_produce(void *threadarg) {
  ...
  while (!done) {
    // get next item
    pthread_mutex_lock(&p->mutex);
    removed = remove_first(&p->head, &p->tail);
    q->remain--;
    pthread_mutex_unlock(&p->mutex);
...
int main(int argc, char *argv[]) {
  ...
  for (t = 0; t < n_consumer; t++) {
    ...
    rc = pthread_create(&threads[t], NULL, thread_consume,
                        &thread_data_array[t]);
    if (rc) {
  ...
  for (t = 0; t < n_producer; t++) {
    ...
    rc = pthread_create(&threads[n_consumer + t], NULL, thread_produce,
                        &thread_data_array[n_consumer + t]);
    if (rc) {
      printf("ERROR; return code from pthread_create() is %d\n", rc);
      exit(-1);
    }
