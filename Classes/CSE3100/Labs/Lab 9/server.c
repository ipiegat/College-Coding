...
void * thread_main(void * arg_in)
{
    ...
    char bob[MSG_BUF_SIZE];
    snprintf(bob, sizeof(bob), "%d\n", gmn_get_max());
    send(sockfd, bob, sizeof(bob), 0);

    char guessC[MSG_BUF_SIZE];
    int received;
    while (gmn_check(&gmn, atoi(guessC))) {

        recv(sockfd, guessC, sizeof(guessC), 0);
        received = atoi(guessC);
        
        if (received < gmn.v) {
            snprintf(bob, sizeof(bob), "%d\n", 1);
            send(sockfd, bob, sizeof(bob), 0);
        }
        else if (received > gmn.v) {
            snprintf(bob, sizeof(bob), "%d\n", -1);
            send(sockfd, bob, sizeof(bob), 0);
        }
    }
