...
int in_dict(char *word)
{
    ...
    while (left <= right) {
        ...
        int comparison = strcmp(word, words[mid]);
...
void decryption(unsigned char key, unsigned char shift, const int *encrypted, int len, char *decrypted)
{
    for (int i = 0; i < len; i++) {
        ...
        char decryptedChar = (value ^ key) >> shift;
...
int message_score(const char *msg)
{
    ...
    char *token = strtok(msg_copy, " "); // Tokenize the message by spaces
    int score = 0;
    ...
    free(msg_copy); // Free the dynamically allocated copy
    return score;
}
...
int read_encrypted(char *filename, int *encrypted)
{
    ...
    while (fread(&encrypted[num_bytes_read], sizeof(int), 1, fp) == 1) {
        num_bytes_read++;
    }
