...
void * printer_main(void * arg)
{
    ...
    printer_t * joe = (printer_t*) arg;
    job_queue_t *jq = joe->jq;
    joe->njobs = 0;
    int done = 0;
...
int main(int argc, char *argv[])
{
    ...
    pthread_t id[num_printers];

    for (int i = 0; i < num_printers; i++) {
        printers[i].id = i;
        printers[i].jq = &job_queue;
        pthread_create(&id[i], NULL, printer_main, &printers[i]);
    }
