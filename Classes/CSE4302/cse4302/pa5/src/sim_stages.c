...
static int current_load_latency = 0;
...
void decode() {
    ...
    int younger_is_branch = younger->valid && (younger->opcode == BTYPE);
    ...
    int older_load_takes_unit = older_is_load && !stall_older && !load_unit_busy;
    int older_store_takes_unit = older_is_store && older->valid && !stall_older;
    ...
    if (older->valid && stall_older) {
        ...
        if (younger_load_raw ||
            ...
            (younger_is_load && older_load_takes_unit) || 
            (older_store_takes_unit)) {
            stall_younger = 1;
        }
    }
    ...
    /* Static taken branch prediction (bp_mode == 1) */
    if (bp_mode == 1) {
        struct State *predicted = NULL;

        /* Only predict on instructions that are actually allowed to proceed */
        if (!stall_older && older_is_branch && older->valid) {
            predicted = older;
        } else if (!stall_younger && younger_is_branch && younger->valid) {
            predicted = younger;
        }

        if (predicted != NULL) {
            unsigned int target = predicted->inst_addr + (unsigned int)predicted->imm;
            advance_pc(target);
            /* Use br_mispredicted to flush both fetch units next cycle */
            br_mispredicted = 1;
        }
    }
...
struct State execute() {
    ...
    switch (opcode) {
        ...
        case BTYPE: {
            ...
            /* --- Control-flow / misprediction handling --- */

            if (bp_mode == 0) {
                /* PA4 behavior: we speculated not-taken.
                 * Mispredict only when branch is actually taken. */
                if (br_taken) {
                    ex_out_n.br_addr = ex_out_n.inst_addr + (unsigned int)ex_out_n.imm;
                    advance_pc(ex_out_n.br_addr);
                    br_mispredicted = 1; /* flush IF/ID */
                }
            } else {
                /* bp_mode == 1: static taken.
                 * Decode already set pc to br_addr for the predicted branch.
                 * Mispredict only when branch is actually NOT taken. */
                if (!br_taken) {
                    /* Correct pc should be fall-through: inst_addr + 4 */
                    ex_out_n.br_addr = ex_out_n.inst_addr + 4u;
                    advance_pc(ex_out_n.br_addr);
                    br_mispredicted = 1; /* flush IF/ID on mispredict */
                }
                /* If br_taken == 1, prediction was correct; we do nothing here. */
            }
...
struct State load_stage() {
    ...
    if (!dmem_busy) {
        ...
        if (dcache_enabled) {
            hit_way = dcache_lookup_2_way(ld_out_n.mem_addr);
        }

        ld_out_n.cache_line_hit_way = hit_way;

        if (dcache_enabled && hit_way != -1) {
            current_load_latency = dcache_access_cycles;
        } else {
            current_load_latency = dmem_access_cycles;
        }
        ...
        dmem_cycles++;
        if (dmem_cycles >= current_load_latency) {
            ...
            if (dcache_enabled) {
                dcache_update_2_way(addr, ld_out.cache_line_hit_way);
                if (ld_out.cache_line_hit_way != -1) {
                    dcache_hits++;
                }
            }
...
struct State store_stage() {
    ...
    if (st_out_n.opcode == STYPE && st_out_n.valid) {
        unsigned int base = st_out_n.alu_in1;
        unsigned int off  = st_out_n.alu_in2;
        unsigned int addr = base + off;
        memory[addr] = st_out_n.mem_buffer;

        if (dcache_enabled) {
            int hit_way = dcache_lookup_2_way(addr);
            dcache_update_2_way(addr, hit_way);
            if (hit_way != -1) {
                dcache_hits++;
            }
        }
...
int dcache_lookup_2_way(unsigned int addr_mem) {
    ...
    /*Insert your code here*/
    unsigned int index = (addr_mem >> 4) & 0x7;
    unsigned int tag = addr_mem >> 7;

    CacheSet_2_way *set = &dcache_2_way[index];

    if (set->block[0].valid && set->block[0].tag == tag) {
        return 0;
    }

    if (set->block[1].valid && set->block[1].tag == tag) {
        return 1;
    }
...
void dcache_update_2_way(unsigned int addr_mem, int line) {
    ...
    /*Insert your code here*/
    unsigned int block_addr = addr_mem / CACHE_LINE_SIZE;
    unsigned int num_sets = (CACHE_SIZE / CACHE_LINE_SIZE) / 2;
    unsigned int set_index = block_addr % num_sets;
    unsigned int tag = block_addr / num_sets;

    CacheSet_2_way *set = &dcache_2_way[set_index];
    int way;

    if (line != -1) {
        way = line;
    } else {
        if (!set->block[0].valid) {
            way = 0;
        } else if (!set->block[1].valid) {
            way = 1;
        } else {
            way = (set->lru_tree == 0) ? 0 : 1;
        }
    }

    set->block[way].valid = 1;
    set->block[way].tag = tag;
